...
dlssg-310.1-d3d12-sm86+sm75 和 sm75_route_limits / sm75_slots），是给 RTX 20 系
...
Core** —— DLSS 帧生成在硬件上就跑不了，换 310.1 版也没用。所以程序判定到 GTX 16 系时会
...
有些用户所有下载源都很慢（实测有 0.02 MB/s 的），那 84 MB 的资产在程序里基本下不完。
...
2. 打开程序 →「上游资产」卡片 → 点 **「导入压缩包…」** → 在文件对话框里把它们**一起选中**
   （支持多选；顺序、文件名、套不套文件夹都无所谓）。已经自己解压过就点「导入文件夹…」。
3. 剩下的程序自己做：递归找需要的文件、逐个校验、放进资产目录，最后给一份逐项清单。
...
| 位置 | 内容 |
|---|---|
| `assets` | 下载的资产（约 79 MB）+ 下载记录 `update_state.json` |
