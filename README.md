...
| 下载 Mod 文件 | 自动在官方源和几个镜像之间挑能用的那个（见下面的「下载速度」） |
...
GitHub 官方源在国内经常几乎不通（实测 0.06 MB/s），所以程序内置了几个加速镜像并
**按实测速度排序**，自动挑能用的那个：
...
| GitHub 官方直连 | 0.00 ~ 0.06 MB/s |

48 MB 资产（代理 DLL + 两个 DLSS 运行库）走最快的镜像只要**十几秒**，走官方源要
十几分钟。程序还会**记住上次成功的源**，下次直接从它开始，不用反复试。

在「上游资产」卡片里可以手动填备用源地址来覆盖这个自动选择；填错也没关系 ——
内容对不上会被自动拒绝（代理 DLL 还会额外校验本项目的数字签名）。

另外「下载 / 更新资产」会先判断本地文件是不是已经是最新版，是的话直接跳过 ——
不会把 15 MB 的代理 DLL 反复拉一遍。判断要同时满足「有下载记录 + 指纹一致 +
文件真的还在且大小对得上」，所以手动删掉资产文件后它会正确地重新下载。

## 检查新版本

启动时会自动检查本工具自己有没有新版本。发现新版本时标题栏会出现一个「有新版本
x.y.z」按钮，点一下打开发布页下载；想手动再查一次，点标题栏的「检查更新」。

这一项和上游 Mod 的资产更新是两回事，所以入口放在标题栏，不在「上游资产」卡片里。

检查方式**不占 GitHub 接口配额**：直接读仓库 main 分支上 Cargo.toml 里的版本号。
（Releases 页面和 releases.atom 都抓不到 —— 加速镜像只代理资源文件、拒绝代理网页。）
